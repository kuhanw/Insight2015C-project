...
def replace_bigrams(input_paragraph):
	...
	for i in range(len(processing_list)):
		print "eliminate:%s" % processing_list[i]
...
my_words = [ "concept", "photo", "like" ,"comes", "things", "com", "don", "help"]#, "improve wisegeek", "related article", "u'improve wisegeek"]
...
coef_path_forest_cv = forest = RandomForestClassifier(n_estimators = 100)
...
plt.figure()
plt.hist2d(binary_y, logistic_prediction)
plt.ylabel("Predicted Values")
plt.xlabel("Truth Values")
plt.title("Logistic Linear Regression")
plt.savefig("figures/logistic_predicted_truth.png")
