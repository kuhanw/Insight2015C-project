...
def feature_extraction_model(widget_selection, Ngram_Range_Low, Ngram_Range_High, Min_DF, PageLoaded, WidgetViewed, ite, Find, input_json_name):
	...
	try: 
		...
		forest_results_parameters_predict_x_test = coef_path_forest_cv.predict(x_test)
		forest_results_parameters_predict_y_test = np.array(make_binary(y_test))

		SGD_results_parameters_predict_x_test = coef_path_SGD_cv.predict(x_test)
		SGD_results_parameters_predict_y_test = np.array(make_binary(y_test))

		logistic_results_parameters_predict_x_test = coef_path_logistic_cv.predict(x_test)
		logistic_results_parameters_predict_y_test = np.array(make_binary(y_test))
		logistic_results_parameters_predict_prob_test = coef_path_logistic_cv.predict_proba(x_test)

	except:

		forest_results_parameters_predict_x_test = np.array([0])
		forest_results_parameters_predict_y_test = np.array([0])

		SGD_results_parameters_predict_x_test = np.array([0])
		SGD_results_parameters_predict_y_test = np.array([0])

		logistic_results_parameters_predict_x_test = np.array([0])
		logistic_results_parameters_predict_y_test = np.array([0])
		logistic_results_parameters_predict_prob_test = np.array([0])
	...
	forest_results_parameters = [ forest_prediction_training, coef_path_forest_cv.get_params, coef_path_forest_cv.feature_importances_, 
					 forest_results_parameters_predict_x_test, forest_results_parameters_predict_y_test, coef_path_forest_cv.classes_] 

	forest_scores = [forest_cv_score, classification_report(binary_y, forest_results_parameters[0]), 'forest',
	  				 precision_score(binary_y, forest_prediction_training),
			                recall_score(binary_y, forest_prediction_training),
			                accuracy_score(binary_y, forest_prediction_training),
			                confusion_matrix(binary_y, forest_prediction_training)
					]
	...
	SGD_results_parameters = [ SGD_prediction_training, coef_path_SGD_cv.get_params, coef_path_SGD_cv.coef_, 
					 SGD_results_parameters_predict_x_test, SGD_results_parameters_predict_y_test] 

	SGD_scores = [SGD_cv_score, classification_report(binary_y, SGD_results_parameters[0]), 'SGD',
					 precision_score(binary_y, SGD_prediction_training),
			                recall_score(binary_y, SGD_prediction_training),
			                accuracy_score(binary_y, SGD_prediction_training),
			                confusion_matrix(binary_y, SGD_prediction_training)
					]
	...
	logistic_results_parameters = [logistic_prediction_training, coef_path_logistic_cv.get_params, coef_path_logistic_cv.coef_, 
					logistic_results_parameters_predict_x_test, logistic_results_parameters_predict_y_test, 
					 logistic_results_parameters_predict_prob_test]
	
	logistic_scores = [logistic_cv_score, classification_report(binary_y, logistic_results_parameters[0]), 'logistic'
					, precision_score(binary_y, logistic_prediction_training), 
					recall_score(binary_y, logistic_prediction_training), 
					accuracy_score(binary_y, logistic_prediction_training),
					confusion_matrix(binary_y, logistic_prediction_training)
					]
